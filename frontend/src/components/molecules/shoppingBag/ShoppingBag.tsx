...
import { totalPrice, cartItems } from "store/selectors";
...
const ShoppingBag = () => {
  ...
  const cartItems = useAppSelector(
    (state) => state.reducer.cartShopSlice.items
  );
  ...
  return (
    <div className={cl.shoppingbag}>
      <Button primary={true}>
        ...
        <p className={`${cl.shoppingbag__cardSummary} normal`}>
          <span>{isMobile ? `${cartItems.length}` : `${totalPrice} ₽`}</span>
        </p>
      </Button>
    </div>
  );
