...
import ShoppingBag from '@features/shoppingBag/shopping-bag';
...
import { headerNavigationSlice } from '@widgets/card-sections/card-sections-navigation/card-sections-navigation.slice';

import styles from './card-sections-navigation.module.scss';
...
const NavigationList = ({ categories }: { categories: Categories }) => {
  ...
  const currentCategories = CATEGORIES_LIST.filter((item) => categoriesList.includes(item.path));
...
export const Navigation = ({ data }) => {
  ...
  return (
    <section className={`flex items-center justify-between  ${styles.headerNavigation}`}>
      <div className={`flex items-center justify-between  ${styles.headerNavigationWrapper}`}>
        <motion.div
          layoutRoot
          className={cn(styles.fixedHeaderWrapper, 'flex items-center justify-between w-full', {
...
const EnchantedHeaderNavigation = withHandleVisibility(Navigation);
const CardSectionsNavigation = ({ data }) => {
  return <EnchantedHeaderNavigation data={data} />;
};

export default CardSectionsNavigation;
